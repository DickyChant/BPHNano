...
def parse_args():
    ...
    p.add_argument('-w', '--workarea', default='ETAP_%s' % PROD_TAG)
    ...
    p.add_argument('-p', '--psetcfg', default='../test/run_etaprime_cfg.py')
    p.add_argument('-m', '--mode', default='ele', choices=['ele', 'lowpt', 'all'],
...
def main():
    ...
    for sname, s in selected.items():
        ...
        mode = args.mode   # e-leg variant: ele|lowpt|all

        for ds in datasets:
            n_tasks += 1
            req = request_name(ds, args.tag)
            pyCfgParams = ['isMC=%s' % isMC, 'variant=%s' % mode, 'globalTag=%s' % gt,
