...
def parse_args():
    ...
    p.add_argument('-w', '--workarea', default='DPIMM_%s' % PROD_TAG)
    ...
    p.add_argument('-p', '--psetcfg', default='../test/run_dpimumu_cfg.py')
...
def main():
    args = parse_args()
    # NB: this test must name the SAME prefix as the --workarea default, or the
    # workarea silently stops following --tag and two campaigns with different tags
    # land in one directory (and one output LFN).  Copying this script from the eta'
    # version left it comparing against 'ETAP_' while the default said 'DPIMM_'.
    if args.workarea == 'DPIMM_%s' % PROD_TAG:   # default workArea follows the campaign tag
        args.workarea = 'DPIMM_%s' % args.tag
