...
def parse_args():
    ...
    p.add_argument('-y', '--yaml', default='samples_upsilon.yml', help='sample list')
    ...
    p.add_argument('-w', '--workarea', default='UPS4L_%s' % PROD_TAG)
    ...
    p.add_argument('-p', '--psetcfg', default='../test/run_upsilon4l_cfg.py')
    p.add_argument('-m', '--mode', default=None, choices=['4mu', '4l'],
    ...
    p.add_argument('--skim', action='store_true', help='keep only events with >=1 Upsilon candidate')
...
def main():
    args = parse_args()
    if args.workarea == 'UPS4L_%s' % PROD_TAG:   # default workArea follows the campaign tag
        args.workarea = 'UPS4L_%s' % args.tag
    ...
    for sname, s in selected.items():
        ...
        mode = args.mode or s.get('mode', '4l')

        for ds in datasets:
            n_tasks += 1
            req = request_name(ds, args.tag)
            pyCfgParams = ['isMC=%s' % isMC, 'mode=%s' % mode, 'globalTag=%s' % gt,
                           'reportEvery=1000', 'maxEvents=%s' % args.maxevents,
                           'nThreads=%d' % args.numcores]
            if args.skim:
                pyCfgParams.append('skim=True')
