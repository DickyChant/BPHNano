...
def parse_args():
    ...
    p.add_argument('-w', '--workarea', default='MMPHI_%s' % PROD_TAG)
    ...
    p.add_argument('-p', '--psetcfg', default='../test/run_mumuphi_cfg.py')
...
def main():
    args = parse_args()
    if args.workarea == 'MMPHI_%s' % PROD_TAG:   # default workArea follows the campaign tag
        args.workarea = 'MMPHI_%s' % args.tag
