...
UpsilonTo2Mu2ETable = cms.EDProducer(
    ...
    variables = cms.PSet(
        ...
        # --- identification, per reconstruction -----------------------------------------
        # BPH-24-001 cuts on the NO-ISOLATION standard-electron MVA; nominal_mva_noiso is that
        # discriminant. The LowPt BDT (low_id / low_unbiased / low_ptbiased) only exists for
        # legs LowPtElectron reconstructed. -999 = the reconstruction is absent.
        trk1_low_id = ufloat('trk1_low_id'), trk2_low_id = ufloat('trk2_low_id'),
        trk1_low_unbiased = ufloat('trk1_low_unbiased'),
        trk2_low_unbiased = ufloat('trk2_low_unbiased'),
        trk1_low_ptbiased = ufloat('trk1_low_ptbiased'),
        trk2_low_ptbiased = ufloat('trk2_low_ptbiased'),
        trk1_nominal_mva_noiso = ufloat('trk1_nominal_mva_noiso'),
        trk2_nominal_mva_noiso = ufloat('trk2_nominal_mva_noiso'),
        trk1_nominal_mva_iso = ufloat('trk1_nominal_mva_iso'),
        trk2_nominal_mva_iso = ufloat('trk2_nominal_mva_iso'),
        trk1_nominal_mva_noiso_wp80 = uint('trk1_nominal_mva_noiso_wp80'),
        trk2_nominal_mva_noiso_wp80 = uint('trk2_nominal_mva_noiso_wp80'),
        trk1_nominal_mva_noiso_wp90 = uint('trk1_nominal_mva_noiso_wp90'),
        trk2_nominal_mva_noiso_wp90 = uint('trk2_nominal_mva_noiso_wp90'),
        # 0 fails veto, 1 veto, 2 loose, 3 medium, 4 tight, -1 no standard reco.
        # NOTE these WPs include an isolation cut; use mva_noiso for an iso-free selection.
        trk1_nominal_cutbased = uint('trk1_nominal_cutbased'),
        trk2_nominal_cutbased = uint('trk2_nominal_cutbased'),
        # cut-based ID inputs, standard leg
        trk1_nominal_einvminuspinv = ufloat('trk1_nominal_einvminuspinv'),
        trk2_nominal_einvminuspinv = ufloat('trk2_nominal_einvminuspinv'),
        trk1_nominal_deta_seed = ufloat('trk1_nominal_deta_seed'),
        trk2_nominal_deta_seed = ufloat('trk2_nominal_deta_seed'),
        trk1_nominal_dphi_in = ufloat('trk1_nominal_dphi_in'),
        trk2_nominal_dphi_in = ufloat('trk2_nominal_dphi_in'),
        trk1_nominal_r9 = ufloat('trk1_nominal_r9'), trk2_nominal_r9 = ufloat('trk2_nominal_r9'),
        # per-reconstruction quality (the unsuffixed ones are for the SELECTED reco only)
        trk1_low_sieie = ufloat('trk1_low_sieie'), trk2_low_sieie = ufloat('trk2_low_sieie'),
        trk1_low_hoe = ufloat('trk1_low_hoe'), trk2_low_hoe = ufloat('trk2_low_hoe'),
        trk1_nominal_sieie = ufloat('trk1_nominal_sieie'),
        trk2_nominal_sieie = ufloat('trk2_nominal_sieie'),
        trk1_nominal_hoe = ufloat('trk1_nominal_hoe'), trk2_nominal_hoe = ufloat('trk2_nominal_hoe'),
        trk1_low_lost_hits = uint('trk1_low_lost_hits'),
        trk2_low_lost_hits = uint('trk2_low_lost_hits'),
        trk1_low_pass_conv_veto = uint('trk1_low_pass_conv_veto'),
        trk2_low_pass_conv_veto = uint('trk2_low_pass_conv_veto'),
        trk1_nominal_lost_hits = uint('trk1_nominal_lost_hits'),
        trk2_nominal_lost_hits = uint('trk2_nominal_lost_hits'),
        trk1_nominal_pass_conv_veto = uint('trk1_nominal_pass_conv_veto'),
        trk2_nominal_pass_conv_veto = uint('trk2_nominal_pass_conv_veto'),
